...
- [x] 新增账号列表、登录、确认、取消、切换、校验、重新登录和删除命令。
- [x] 登录与校验使用独立长超时，普通命令保持当前短超时。
- [x] Python JSON 错误完整映射到前端，不泄露 Cookie、路径中的密钥或网页凭据。
- [x] 运行 `cargo fmt --check`、`cargo check` 和 Rust 测试。
...
- [x] 增加一级导航“账号”，四个平台用紧凑列表显示账号、状态和当前标记。
- [x] 支持添加、候选确认、取消、快速切换、校验、重新登录和删除。
- [x] 添加账号时说明将打开独立 Edge 登录窗口，候选身份必须二次确认。
- [x] 在运行与来源页面显示任务将使用的平台和当前账号。
- [x] 加载、空状态、超时、登录失效、冲突和删除当前账号均有完整交互。
- [x] 使用现有图标库和现有桌面设计语言，不增加软件外入口。
- [x] 运行前端类型检查和生产构建，并通过真实 Tauri 窗口检查桌面布局。
...
- [x] 运行全部 Python 单元测试。
- [x] 运行 `doctor`、前端构建、Rust 格式/检查/测试。
- [x] 迁移并验证现有抖音账号，执行一次真实收藏获取与完整入库回归。
- [x] 验证哔哩哔哩、YouTube、TikTok 的登录入口、账号识别和采集鉴权路径；没有现成登录态时明确记录需要用户在平台页面完成登录，不伪造成功。
- [x] 构建 Windows 独立安装包，并在干净工作目录验证启动和账号页面。
- [x] 检查日志和临时目录，确认 Cookie 临时文件已清除、日志无凭据。

## Task 11：全项目复查、修复与发布

- [x] 按代码审查标准检查账号安全、并发、错误恢复、升级兼容、UI 状态和测试盲区。
- [x] 对发现的问题逐项补测试并修复，再重新执行全部验证。
- [x] 更新最终合并说明，使用自然中文描述，不出现 AI 痕迹。
- [x] 提交并推送全部变更。
- [x] 创建并推送 `v0.04` 标签，发布 GitHub Release 与 Windows 安装包。
- [x] 最终报告真实完成项、验证结果、安装包位置和仍需平台网页人工登录的客观边界。
