...
- [ ] 先写失败测试，覆盖列表、开始登录、候选确认、取消、切换、校验、重新登录和删除。
- [ ] 登录命令等待浏览器登录成功后只保存候选，不直接替换当前账号。
- [ ] 确认命令写入账号并按用户选择切换；取消命令清理候选 profile。
- [ ] 自动迁移现有抖音登录态，识别为“忆霖 / 60185413619”，迁移前保留原配置备份。
- [ ] CLI 所有结果使用稳定 JSON，错误包含平台、错误码和可执行修复提示。
- [ ] 运行服务与 CLI 测试并提交。
...
- [ ] 先写失败测试，验证每个需要登录的平台都使用当前账号。
- [ ] 抖音运行时从当前账号生成临时下载配置。
- [ ] 哔哩哔哩、YouTube、TikTok 的 yt-dlp 列表与下载命令传入临时 Cookie。
- [ ] 未配置账号、账号失效、导出失败时拒绝静默降级，并返回明确提示。
- [ ] 无需登录的普通网页、PDF、播客和本地文件保持原有路径。
- [ ] 运行采集和流水线测试并提交。
