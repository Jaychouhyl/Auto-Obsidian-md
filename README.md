...
本项目按本地桌面工具设计：用户自己配置 API Key，资料默认保存在本机，不提供云同步、不托管账号、不保存平台密码。

抖音、B 站、YouTube、TikTok 等平台来源依赖用户当前登录态、平台页面、下载器适配和平台规则；软件只处理用户当前可访问的内容，不承诺永久稳定抓取全部收藏或绕过平台限制。详细说明见 [平台能力边界](docs/user/平台能力边界.md)。
